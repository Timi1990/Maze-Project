...
public class MyMaze3dGenerator extends Maze3dGeneratorBase {
	...
	private void dfs(int z,int y,int x)
	{
		...
		for(int n:direction)
		{
			...
			break;
			case 3:if(y+2>=rows-1||(checkIfStart(x, y, z)))     //Forward
				continue;
			else
			{
				if(arr[z][y+2][x]!=0)
			...
			break;
			case 4:if(y-2<=0||(checkIfStart(x, y, z)))      //Back
				continue;
			else
			{
				if(arr[z][y-2][x]!=0)
			...
			break;
			case 6:if(z-1<=0||checkIfStart(x, y, z))      //Down
				continue;
			else
			{
				if(arr[z-1][y][x]!=0)
